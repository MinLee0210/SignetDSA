//! Known-answer tests for Schnorr (BIP340) against the official secp256k1 test
//! vectors published at
//! <https://github.com/bitcoin/bips/blob/master/bip-0340/test-vectors.csv>.
//!
//! Only the fields needed for verification are used (public key, message,
//! signature, expected result) — this exercises both the valid-signature
//! paths and BIP340's documented invalid/malleability edge cases (public key
//! not on the curve, negated `s`, non-canonical field elements, etc.).
...
// Transcribed verbatim from the official BIP340 CSV (index column noted in comments).
const VECTORS: &[Vector] = &[
    ...
    // 9 — sG - eP is infinite (x(inf) as 0 case).
    Vector {
        public_key: "DFF1D77F2A671C5F36183726DB2341BE58FEAE1DA2DECED843240F7B502BA659",
        message: "243F6A8885A308D313198A2E03707344A4093822299F31D0082EFA98EC4E6C89",
    ...
    // 10 — sG - eP is infinite (x(inf) as 1 case).
    Vector {
        public_key: "DFF1D77F2A671C5F36183726DB2341BE58FEAE1DA2DECED843240F7B502BA659",
        message: "243F6A8885A308D313198A2E03707344A4093822299F31D0082EFA98EC4E6C89",
...
#[test]
fn schnorr_matches_bip340_test_vectors() {
    for (i, vector) in VECTORS.iter().enumerate() {
        let pk_bytes = from_hex(vector.public_key);
        let sig_bytes = from_hex(vector.signature);
        let message = from_hex(vector.message);

        // Parsing itself is allowed to fail for the negative vectors (e.g. a
        // public key that isn't a valid curve point, or a signature whose
        // components aren't canonical field elements) — that's still a
        // correct rejection.
        let result = VerifyingKey::from_bytes(&pk_bytes).and_then(|vk| {
        ...
        assert_eq!(
            result.is_ok(),
            vector.valid,
            "BIP340 vector {i} disagreed with expected result {}",
            vector.valid
        );
    }
}
