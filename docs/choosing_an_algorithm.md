...
- **Verification cost for BLS** is a pairing operation — slower per-signature
  than any other algorithm here in isolation, but that's the tradeoff for
  aggregation: verifying *N* aggregated BLS signatures in one pairing check
  can still beat *N* separate ECDSA/EdDSA verifications.
